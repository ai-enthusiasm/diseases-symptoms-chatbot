...
function clearMessageHistory() {
	$.ajax({
		type: 'POST',
		url: 'http://127.0.0.1:5000/clear_messages_history',
		success: function(response) {
			console.log('Message history cleared:', response);
		},
...
async function send(text) {
    ...
    try {
        ...
        await $.ajax({
            type: 'POST',
            url: 'http://127.0.0.1:5000/save_messages_history',
        ...
        // Process the response from inference API
        console.log('Inference response:', data);
        newRecievedMessage(data);
    } catch (error) {
        console.error('Error:', error);
        if (error.status === 404) {
            newRecievedMessage("Bác sĩ hiện đang vắng, vui lòng quay lại sau.");
        }
    }

    // Store message to DB (assuming this is a synchronous operation)
    storeMessageToDB();
}
...
function createNewMessage(message) {
	...
	$chatlogs.append(
		$('<div/>', {'class': 'chat friend'}).append(
			$('<div/>', {'class': 'user-photo'}).append($('<img src="../../Images/avt.png" />')),
